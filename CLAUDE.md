...
現状の最大のボトルネックは**PV不足、その正体は「載ってはいるがクリックされる位置にいない」こと**。
2026-09-20のSearch Console実測で、**インデックス未登録問題は解決済み**と確認した（登録済み41／未登録30だが内訳はリダイレクト16・canonical代替13・noindex1で、**「検出-インデックス未登録」と「クロール済み-未登録」は0本**＝登録すべきページは全部登録されている）。
数字の現在地は週あたり**表示538・クリック4・CTR 0.7%・平均16.1位**。つまり課題は発見されることではなく**順位とCTR**。
技術的なSEO（sitemap・canonical・内部リンク・noindex申告）はすでに整っており、**コードで直せる余地はほぼない**。
したがって「SEOのためにこれを直しましょう」系の提案をする前に、それが本当に順位／流入の増加につながるかを一度考えること。効くのは**①競合の手薄なクエリを取りにいく新規コンテンツ ②すでに1ページ目下段にいるページを上に押し込むこと**の2つ。

**何が効いて何が効かないかの実測（2026-09-20時点）**

| 施策 | 結果 |
|---|---|
| `/column/squat-average`（スポーク記事） | 公開5日で62表示・**10.5位**、`スクワット 平均` で**9.0位**。競合が手薄なクエリ＝効く |
| `/column/bench-press-average`（同じ型） | 11日で50表示だが**24.7位**、11クエリに散って1つも上位なし。競合が強い領域＝効きにくい |
| `/weight-checker` に静的表を追加 | 9.0位で頭打ち。表示は増えたがクリックは出ていない |
| `/rm-calculator` に静的表を追加 | **95.9位**。勝てていない領域に表を足しても動かない、の実例 |
| ハブ＋スポーク構成のカニバリ | **起きていない**。ハブは「ビッグ3/big3」系、スポークは「スクワット○○」系でクエリが分離した |
...
2. `AuthorBox` を記事末尾に置く（37本中37本が設置済み。E-E-A-Tの一貫性のため例外を作らない）
3. `ShareButtons` を置く
